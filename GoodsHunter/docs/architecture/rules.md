...
9. **专用词，词表参考**：在生成代码，字段名称之前，需要先参考已有的词表和概念。见：`GoodsHunter/docs/architecture/vocabulary.md`。若新增了概念，名词，也需要在表中增加。
10. **渐进式披露**：文档编写的规则采用渐进式披露的准则，即文档之间可以互相引用，引用之处阐述目标文档是什么作用，有什么内容，让LLM自行决定是否要阅读和引用对应的文档。
## 
